from finetune_apps_utils import *
from pretrain_utils import set_seed, Logger, set_tokenizer, CycleIndex, save_ckpt
from finetune_translation_utils import load_model
import os
import torch
...
def main():
    ...
    train_data, test_data = data_by_split['train'], data_by_split['test']
    
    # load model
    model = load_model(args, 'data/apps_generation/all_node_types.pkl')
    ...
    if args.do_train:
        ...
        for step in train_bar:
            ...
            # generate
            if ((step+1)>=args.generate_after) and ((step+1)%args.generate_every==0):
                # get metrics on validation set
                generate(model, test_data, args, train_step=step, split='test')
                model.train()          
    ...
    if args.do_test:
        results = generate(model, test_data, args, split='test')
